...
import { fetchProtectedTenantConfigList } from '../services/configService';
...
async function handleFetchTenantConfig() {
  ...
  try {
    ...
    const response = await fetchProtectedTenantConfigList({
      tenantId,
      accessToken,
    });
...
<template>
  <section class="page">
    <h1>Tenant Config List</h1>

    <div class="info-card">
      <p>
        Fetch the list-style tenant configuration from the protected Config Service endpoint using your Auth0 access token.
      </p>
